<?php
    ...
    if(isset($_POST['nick'])){  
        ...
        //czy walidacja udana? usatwienie false gdy coś pójdzie nie tak
        $validation = true;

        //sprawdzamy poprawnosc nick
        $nick = $_POST['nick'];
        //sprawdzenie dlgosci nicka
        if((strlen($nick)<3) || (strlen($nick)>20)){
            $validation = false;
            $_SESSION['errNick'] = "Imię musi posiadać od 3 do 20 znaków.";
        }
        //sprawdzenie czy wszystkie znaki są alfanumeryczne
        if(ctype_alnum($nick)==false){
            $validation = false;
            $_SESSION['errNick'] = "Nick może składać się tylko z liter i cyfr (bez polskich znaków)";
        }

        //sprawdzamy poprawnosc emaila
        $email = $_POST['email']; 
        //filtr ze stałą która usuwa wszystkie niedozwolone znaki z adresu email
        $email2 = filter_var($email, FILTER_SANITIZE_EMAIL);
        //kolejny filtr do przefilrtowanego już email2 i sprawdzenie po sanityzacji poprawności z email
        if((filter_var($email, FILTER_VALIDATE_EMAIL)==false) || ($email2 != $email)){
            $validation = false;
            $_SESSION['errEmail'] = "Nieprawidłowy adres e-mail.";
        }

        //sprawdzamy poprawność hasła
        $password = $_POST['password'];
        //sprawdzamy długość hasła min 6 , max 20
        if((strlen($password)<6) || (strlen($password)>20)){
            $validation = false;
            $_SESSION['errPassword'] = "Hasło musi zawierać od 6 do 20 znaków.";
        }
...
<head>
    ...
    <title>Registration Personal Budget</title>
        ...
        <article>
            <div>
                <blockquote class="blockquote d-block w-100">
                    <h2 class="text-center p-5">Wprowadź potrzebne dane i zarejestruj:</h2>
                </blockquote>
            </div>
            <div>
                <form method="post">
                    <div class="d-flex align-items-center flex-column  ">
                        <div class="input-group col-md-4 col-7 m-1">
                            ...
                            <input type="text" class="form-control" placeholder="imię" name="nick">
                            <?php
                                if(isset($_SESSION['errNick'])){
                                    echo '<div class="d-flex justify-content-center text-danger">'.$_SESSION['errNick'].'</div>';
                        ...
                        <div class="input-group col-md-4 col-7 m-1">
                            ...
                            <?php
                                if(isset($_SESSION['errEmail'])){
                                    echo '<div class="d-flex justify-content-center text-danger">'.$_SESSION['errEmail'].'</div>';
                        ...
                        <div class="input-group col-md-4 col-7 m-1">
                            ...
                            <?php
                                if(isset($_SESSION['errPassword'])){
                                    echo '<div class="d-flex justify-content-center text-danger">'.$_SESSION['errPassword'].'</div>';
                    ...
                    <div class="g-recaptcha d-flex justify-content-center col-md-4 col-7 m-2" data-sitekey="6LcA9_khAAAAAEUqRHu7pPjx6CD19bPYf1KKlRef"></div>
                    </div>
                    <?php
                        if(isset($_SESSION['errBot'])){
                            echo '<div class="d-flex justify-content-center text-danger">'.$_SESSION['errBot'].'</div>';
